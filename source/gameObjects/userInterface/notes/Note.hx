...
class Note extends FNFSprite
{
	...
	static function noteTypeDir(type:Int, backDir:Bool = false):String // Look, I'm just doing a lazy port
	{
		switch (type)
		{
			case 1:
	...
	public static function returnDefaultNote(assetModifier, strumTime, noteData, noteType, noteAlt, ?isSustainNote:Bool = false, ?prevNote:Note = null):Note
	{
		...
		switch (assetModifier)
		{
			...
			default: // base game arrows for no reason whatsoever
				newNote.frames = Paths.getSparrowAtlas(ForeverTools.returnSkinAsset(noteTypeDir(isSustainNote ? 0 : noteType), assetModifier, Init.trueSettings.get("Note Skin"),
					'noteskins/' + noteTypeDir(isSustainNote ? 0 : noteType, true)));
				...
				if (isSustainNote)
				{
					if (noteType == 2)
					{
						newNote.animation.addByPrefix('purpleholdend', 'red hold end');
						newNote.animation.addByPrefix('greenholdend', 'red hold end');
						newNote.animation.addByPrefix('redholdend', 'red hold end');
						newNote.animation.addByPrefix('blueholdend', 'red hold end');
						newNote.animation.addByPrefix('purplehold', 'red hold piece');
						newNote.animation.addByPrefix('greenhold', 'red hold piece');
						newNote.animation.addByPrefix('redhold', 'red hold piece');
						newNote.animation.addByPrefix('bluehold', 'red hold piece');
					}
					else
					{
