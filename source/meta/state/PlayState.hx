...
class PlayState extends MusicBeatState
{
			...
			for (strumline in strumLines)
			{
				...
				strumline.allNotes.forEachAlive(function(daNote:Note)
				{
					...
					if (!daNote.tooLate && daNote.strumTime < Conductor.songPosition - (Timings.msThreshold) && !daNote.wasGoodHit)
					{
						...
						{
							var basic:Bool = (daNote.noteType == 0 || daNote.noteType == 2);
							if (!daNote.isSustainNote && basic)
							{
								daNote.tooLate = true;
							...
							else if (daNote.isSustainNote && basic)
							{
	...
	private function mainControls(daNote:Note, char:Character, strumline:Strumline, autoplay:Bool):Void
	{
		...
		if (autoplay)
		{
			...
			if (daNote.strumTime <= Conductor.songPosition && (daNote.noteType == 0 || daNote.noteType == 2 || (!daNote.mustPress && char.curCharacter == 'obama'))) // The daNote.mustPress due to dev intention of using death notes for the opponent to play
			{
	...
	private function strumCameraRoll(cStrum:FlxTypedGroup<UIStaticArrow>, mustHit:Bool)
	{
		...
		{
			var camDisplaceExtend:Float = 24;
