...
class OriginalChartingState extends MusicBeatState
{
	...
	**/
	...
	private function addNote():Void
	{
		...
		_song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteSus, false, 0, noteType]);

		curSelectedNote = _song.notes[curSection].sectionNotes[_song.notes[curSection].sectionNotes.length - 1];

		if (FlxG.keys.pressed.CONTROL)
		{
			_song.notes[curSection].sectionNotes.push([noteStrum, (noteData + 4) % 8, noteSus, false, 0, noteType]);
		}
